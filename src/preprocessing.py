...
def feature_engineering(df):
    ...
    # 3600 seconds = 1 hour
    # Our time column has been created by seconds that would be make some problems for models so that we are turning it into hour after doing that we won't need time column anymore and finally we are dropping it from dataset
...
def scale_data(df):
    scaler = RobustScaler() # This scaler is specifically chosen because it handles outliers effectively
