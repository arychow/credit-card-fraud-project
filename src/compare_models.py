...
def compare_all_models():
    ...
    for name, model in models.items():
        ...
        # In the original credit card fraud data set we have really small fraud ratio. To avoid Data Leakage and losing performance we are using ImbPipeline method.(ImbalancedPipeline)
        ...
        # for fraud detection (Accuracy Paradox).a
        # We are using cross validation method because after doing it in for loop we are getting the mean of their own accuracy one by one to make comparision between each other
