...
def evaluate_model(model, X_test, y_test, output_dir):
    ...
    # Because getting probabilities allow us more flexible threshold tuning later
    ...
    print(f"Optimal Threshold found: {best_threshold}") # It iterates through all possible threshold values, calculates the F1 Score for each, and mathematically identifies the "Optimal Threshold" (best_threshold) that maximizes model performance, rather than using the default 0.5.
    ...
    #It plots the ROC Curve, visualizing the trade-off between sensitivity (True Positive Rate) and specificity (False Positive Rate). A higher AUC score (area under the curve) indicates a better model.
